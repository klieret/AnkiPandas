...
def update_card_indices(db: sqlite3.Connection):
    cur = db.cursor()
    cur.execute('CREATE INDEX IF NOT EXISTS idx_cards_odid ON cards (odid) WHERE odid != 0')
    cur.execute('CREATE INDEX IF NOT EXISTS ix_cards_nid on cards (nid)')
    cur.execute('CREATE INDEX IF NOT EXISTS ix_cards_sched on cards (did, queue, due)')
